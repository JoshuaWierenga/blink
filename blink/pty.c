...
static char32_t *GetXlatAscii(void) {
  unsigned i;
  static bool once;
  static char32_t xlat[128];
  if (!once) {
    for (i = 0; i < 128; ++i) {
      xlat[i] = i;
    }
...
static char32_t *GetXlatLineDrawing(void) {
  unsigned i;
  static bool once;
  static char32_t xlat[128];
  if (!once) {
...
static void XlatAlphabet(char32_t xlat[128], int a, int b) {
  unsigned i;
  for (i = 0; i < 128; ++i) {
    if ('a' <= i && i <= 'z') {
      xlat[i] = i - 'a' + a;
...
static char32_t *GetXlatItalic(void) {
  static bool once;
  static char32_t xlat[128];
  if (!once) {
    XlatAlphabet(xlat, U'𝑎', U'𝐴');
    once = true;
  }
  return xlat;
}

static char32_t *GetXlatBoldItalic(void) {
  static bool once;
  static char32_t xlat[128];
  if (!once) {
    XlatAlphabet(xlat, U'𝒂', U'𝑨');
    once = true;
  }
  return xlat;
}

static char32_t *GetXlatBoldFraktur(void) {
  static bool once;
  static char32_t xlat[128];
  if (!once) {
    XlatAlphabet(xlat, U'𝖆', U'𝕬');
    once = true;
  }
  return xlat;
}

static char32_t *GetXlatFraktur(void) {
  unsigned i;
  static bool once;
  static char32_t xlat[128];
  if (!once) {
    for (i = 0; i < ARRAYLEN(xlat); ++i) {
      if ('A' <= i && i <= 'Z') {
        xlat[i] = U"𝔄𝔅ℭ𝔇𝔈𝔉𝔊ℌℑ𝔍𝔎𝔏𝔐𝔑𝔒𝔓𝔔ℜ𝔖𝔗𝔘𝔙𝔚𝔛𝔜ℨ"[i - 'A'];
      } else if ('a' <= i && i <= 'z') {
        xlat[i] = i - 'a' + U'𝔞';
      } else {
        xlat[i] = i;
...
static char32_t *GetXlatDoubleWidth(void) {
  unsigned i;
  static bool once;
  static char32_t xlat[128];
  if (!once) {
...
static char32_t *GetXlatSgr(struct Pty *pty) {
...
static void PtySetXlat(struct Pty *pty, char32_t *xlat) {
  pty->xlat = xlat;
...
static uint32_t *u32set(uint32_t *p, uint32_t c, size_t n) {
  size_t i;
  for (i = 0; i < n; ++i) {
    p[i] = c;
  }
