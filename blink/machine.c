...
static void AluwFlipRo(struct Machine *m, uint32_t rde, const aluop_f ops[4]) {
  ops[RegLog2(rde)](
      Read64(RegRexrReg(m, rde)),
      ReadMemory(rde, GetModrmRegisterWordPointerReadOszRexw(m, rde)),
      &m->flags);
...
static void Aluwi(struct Machine *m, uint32_t rde, const aluop_f ops[4]) {
  uint8_t *a;
  a = GetModrmRegisterWordPointerWriteOszRexw(m, rde);
  WriteRegisterOrMemory(
      rde, a,
...
static void AluwiRo(struct Machine *m, uint32_t rde, const aluop_f ops[4]) {
  ops[RegLog2(rde)](
      ReadMemory(rde, GetModrmRegisterWordPointerReadOszRexw(m, rde)),
...
static uint64_t AluPopcnt(struct Machine *m, uint32_t rde, uint64_t x) {
...
static uint64_t AluBsr(struct Machine *m, uint32_t rde, uint64_t x) {
  unsigned n;
  if (Rexw(rde)) {
...
static uint64_t AluBsf(struct Machine *m, uint32_t rde, uint64_t x) {
  unsigned n;
  if (Rexw(rde)) {
    x &= 0xffffffffffffffff;
    n = 64;
...
static void Bitscan(struct Machine *m, uint32_t rde,
                    uint64_t op(struct Machine *, uint32_t, uint64_t)) {
  WriteRegister(
      rde, RegRexrReg(m, rde),
      op(m, rde,
         ReadMemory(rde, GetModrmRegisterWordPointerReadOszRexw(m, rde))));
...
static void OpLdmxcsr(struct Machine *m, uint32_t rde) {
  m->mxcsr = Read32(ComputeReserveAddressRead4(m, rde));
}

static void OpStmxcsr(struct Machine *m, uint32_t rde) {
  Write32(ComputeReserveAddressWrite4(m, rde), m->mxcsr);
}
