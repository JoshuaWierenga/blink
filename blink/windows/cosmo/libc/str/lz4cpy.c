...
#include "blink/windows/cosmo/libc/intrin/pushpop.h"
#include "blink/windows/cosmo/libc/intrin/repmovsb.h"

// Based on https://github.com/jart/cosmopolitan/blob/9634227/libc/str/lz4cpy.c
...
void *lz4cpy(void *dest, const void *blockdata, size_t blocksize) {
  ...
  for (op = dest, ip = (void*)blockdata, ipe = ip + blocksize;;) {
    token = *ip++;
    length = token >> 4;
    fifteen = pushpop(15);
    if (length == fifteen) {
